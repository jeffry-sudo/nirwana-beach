...
class Laporan_model_parkir extends CI_Model {
    ...
    public function get_filtered_data($dari_tanggal, $sampai_tanggal) {
        ...
        {
            $this->db->where('kd_admin is not null');
        }else{
            $this->db->where('kd_admin="'.$this->session->userdata('kd_admin').'"');
        }
        ...
        $harga_per_mobil = 1000;
        $harga_per_motor = 1000;
        $harga_per_orang = 1000;
        ...
        $this->db->select('nama_kendaraan, jml_org, tgl_masuk, create_masuk, kd_masuk, create_masuk');
        $this->db->from('tbl_masuk a');
        $this->db->join('tbl_kendaraan b', 'a.kd_kendaraan = b.kd_kendaraan');
        $this->db->where('a.kd_kendaraan in ("JK001","JK002")');
         if($this->session->userdata('level')==1)
        {
            $this->db->where('kd_admin is not null');
        }else{
        ...
        //detail
        $this->db->select('c.nama_admin, SUM( CASE WHEN a.kd_kendaraan = "JK002" THEN 1000 WHEN a.kd_kendaraan = "JK001" THEN 1000 ELSE 0 END ) AS total_penjualan');
        $this->db->from('tbl_masuk a');
        $this->db->join('tbl_kendaraan b', 'a.kd_kendaraan = b.kd_kendaraan','left');
        $this->db->join('tbl_admin c', 'a.kd_admin = c.kd_admin','left');
        $this->db->group_by('1');

         if($this->session->userdata('level')==1)
        {
            $this->db->where('a.kd_admin is not null');
        }else{
            $this->db->where('a.kd_admin="'.$this->session->userdata('kd_admin').'"');
        }
